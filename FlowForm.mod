...
subject to SumY1: sum{j in CITIES: ord(j) !=1} y[1,j] = n-1;
subject to SumYj {j in CITIES: ord(j) !=1} : 
	(sum{i in CITIES:ord(i) != ord(j)} y[i,j]) - (sum{k in CITIES:ord(j) != ord(k)} y[j,k])= 1;
