...
subject to Flow {i in CITIES, j in CITIES: ord(i) != ord(j)}: y[i,j] <= (n-1)*x[i,j];
subject to SumY1: sum{j in CITIES: j !=1} y[1,j] = n-1;
subject to SumYj {j in CITIES: j !=1} : sum{i in CITIES:ord(i) != ord(j)} y[i,j] - sum{k in CITIES:ord(k) != ord(j)} y[j,k]=1;